...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    records.forEach(record => {
      if (record.Hydraulic_Oil && Array.isArray(record.Hydraulic_Oil.value)) {
        record.Hydraulic_Oil.value.forEach(sub => {
          if (sub.value.Tellus?.value === "Tellus 32") {
    ...
    console.log('Total Tellus 32 rows:', tellusRows.length);
...
// This function fetches Tellus 32 refill data from Kintone for a specific month and year
