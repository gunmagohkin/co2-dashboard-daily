        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (isOperatingDay) {
                records.push({
                    ...
                    "Consumption_Category": { "value": "EP-220" },
                    "Plant_Location": { "value": "GGPC - Gunma Gohkin" },
                    "Total_Consumed_EP": { "value": (Math.random() * 20).toFixed(2) },
                    "Machine_Run": { "value": Math.floor(Math.random() * 8) + 1 },
                    "Remaining_Stock_EP": { "value": (100 - day * 2).toFixed(2) },
                    "Delivery_EP": { "value": (day % 10 === 0) ? '1' : '0' },
                    "Refill_EP": { "value": (day % 5 === 0) ? '1' : '0' },
                    "Total_Stock_EP_Pail": { "value": '5' },
                    "Total_Stock_EP_Lit": { "value": '90' }
                });
            }
        }
        return { records };
  ...
  function aggregateDataByWeek(records, daysInMonth, selectedYear, selectedMonth, config) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if(rec){
        ...
        weeklyData[week].totalRefills += parseFloat(rec['Refill_EP']?.value) || 0;
      }
    }
